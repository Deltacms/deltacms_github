...
<div id="formConfigCopy" class="displayNone">
	<div class="formConfigInput">
		...
		<div class="row">
			...
			<div class="col5">
				<?php echo template::text('formConfigName[]', [
					'placeholder' => $text['guestbook_view']['config'][0]
				]); ?>
			</div>
			<div class="col4">
		...
		<div class="formConfigMoreLabel displayNone">
			<?php echo template::label('formConfigLabel', $text['guestbook_view']['config'][1], [
					'class' => 'displayNone formConfigLabelWrapper'
				]); ?>
		</div>
		<div class="formConfigMore displayNone">
			<?php echo template::text('formConfigValues[]', [
				'placeholder' => $text['guestbook_view']['config'][2],
				'class' => 'formConfigValues',
				'classWrapper' => 'displayNone formConfigValuesWrapper'
			]); ?>
			<?php echo template::checkbox('formConfigRequired[]', true, $text['guestbook_view']['config'][3]); ?>
		</div>
	</div>
</div>
	...
	<div class="row">
		<div class="col2">
			<?php echo template::button('formConfigBack', [
				...
				'ico' => 'left',
				'value' => $text['guestbook_view']['config'][4]
			]); ?>
		</div>
		<div class="col2">
			<?php echo template::button('formIndexHelp', [
				'class' => 'buttonHelp',
				'ico' => 'help',
				'value' => $text['guestbook_view']['config'][39]
			]); ?>
		</div>
		<div class="col2 offset2" <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo '<div style="display: none;">'; ?>>
			<?php echo template::button('formConfigTexts', [
				...
				'value' => $text['guestbook_view']['config'][43].' '.template::flag($flag, '20px')
			]); ?>
		</div>
		<div class="col2" <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo '<div style="display: none;">'; ?>>
			<?php echo template::button('formConfigData', [
				...
				'value' => $text['guestbook_view']['config'][5]
			]); ?>
		</div>
		<div class="col2 <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo 'offset6'; ?>">
			<?php echo template::submit('formConfigSubmit',[
				'value' => $text['guestbook_view']['config'][30]
			]); ?>
		</div>
	</div>
	<!-- Aide à propos de la configuration de formulaire, view config -->
	<div class="helpDisplayContent">
		<?php echo file_get_contents( $text['guestbook_view']['config'][40]) ;?>
	</div>
	<div class="row">
		<div class="col12">
			<div class="block">
				<div class="blockTitle"><?php echo $text['guestbook_view']['config'][6]; ?></div>
				<div <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo '<div style="display: none;">'; ?> >
					<?php echo template::checkbox('formConfigMailOptionsToggle', true, $text['guestbook_view']['config'][9], [
						...
						<div class="row">
							...
							<div class="col4">
								<?php echo template::text('formConfigMail', [
									'label' => $text['guestbook_view']['config'][16],
									'value' => $this->getData(['module', $this->getUrl(0), 'config', 'mail']),
									'help' => $text['guestbook_view']['config'][17]
								]); ?>
							</div>
						</div>
						<div class="row">
							<div class="col6 offset1">
								<?php echo template::checkbox('formConfigMailReplyTo', true, $text['guestbook_view']['config'][18], [
										'checked' => (bool) $this->getData(['module', $this->getUrl(0), 'config', 'replyto']),
										'help' => $text['guestbook_view']['config'][19]
									]); ?>
							</div>
						</div>
					</div>
				...
				<div class="row" <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo '<div style="display: none;">'; ?>>
					<div class="col6">
						<?php echo template::checkbox('formConfigPageIdToggle', true, $text['guestbook_view']['config'][24], [
					...
					<div class="col5">
						<?php echo template::select('formConfigPageId', $module::$pages, [
							...
							'label' => $text['guestbook_view']['config'][25],
				...
				<div class="row" <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo '<div style="display: none;">'; ?>>
					<div class="col4">
						<?php echo template::checkbox('formConfigCaptcha', true, $text['guestbook_view']['config'][26], [
							'checked' => $this->getData(['module', $this->getUrl(0), 'config', 'captcha'])
						]); ?>
					</div>
					<div class="col8">
						<?php echo template::checkbox('formConfigRgpdCheck', true, $text['guestbook_view']['config'][41], [
							'checked' => $this->getData(['module', $this->getUrl(0), 'config', 'rgpdCheck']),
							'help' => $text['guestbook_view']['config'][42]
						]); ?>
					</div>
				</div>
			</div>
			<div class="block" <?php if($this->getUser('group') < self::GROUP_MODERATOR) echo '<div style="display: none;">'; ?>>
				<div class="blockTitle"><?php echo $text['guestbook_view']['config'][27]; ?></div>
				<div id="formConfigNoInput">
					<?php echo template::speech($text['guestbook_view']['config'][28]); ?>
				</div>
...
<div class="moduleVersion"><?php echo $text['guestbook_view']['config'][29]; ?>
