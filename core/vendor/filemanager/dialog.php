...
<!-- header div end -->
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			$memTooHeavyDisplayMessage = false;
			$memVeryHeavyDisplayMessage = false;
			foreach ($files as $nu=>$file_array) {
				...
				if(in_array($file_array['extension'], $config['ext_img'])){
					...
					if($ftp){
						...
						if (!file_exists($src_thumb)) {
							if (!create_img($file_path, $creation_thumb_path, 122, 91, 'auto', $config)) {
								$src_thumb = $mini_src = "";
							}
						}
				...
				// code heavy
				$tooHeavyDisplayMessage = false;
				$veryHeavyDisplayMessage = false;
				if($is_img){
					$size = filesize($currentPath.$subdir.$file);
					if($size > $config['img_too_heavy']){
						if( $size > $config['img_very_heavy']){
							$veryHeavyDisplayMessage = true;
							$memVeryHeavyDisplayMessage = true;
						} else {
							$tooHeavyDisplayMessage = true;
							$memTooHeavyDisplayMessage =true;
						}
					}
				}
?>
			...
			<?php if($config['multiple_selection']){ ?><div class="selector">
				...
				<div class="img-precontainer">
					...
					<div class="img-container">
						...
						<?php if($tooHeavyDisplayMessage){ ?><span class="img_heavy too_heavy"></span><?php } ?>
						<?php if($veryHeavyDisplayMessage){ ?><span class="img_heavy very_heavy"></span><?php } ?>
					</div>
				</div>
				...
				<figcaption>
					...
					<?php // Prévisualisations
					if($is_img && $src_thumb!=""){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="img-viewer.php?img=<?=$rfm_subfolder.$subdir.$file?>"
					href="javascript:void('');"><i class=" icon-eye-open"></i></a>
					<?php } elseif(($is_video || $is_audio) && in_array($file_array['extension'],$config['media_viewer_exts'])){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="media-viewer.php?media=<?=$rfm_subfolder.$subdir.$file?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					<?php } elseif($config['preview_text_files'] && in_array($file_array['extension'],$config['previewable_text_file_exts'])){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="texte-info.php?info=<?=$rfm_subfolder.$subdir.$file?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					<?php } elseif($config['preview_doc_enabled'] && in_array($file_array['extension'],$config['preview_doc_file_exts'])){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo trans('Preview')?>" data-url="object-viewer.php?object=<?=$rfm_subfolder.$subdir.$file?>"
					href="javascript:void('');"><i class=" icon-eye-open"></i></a>
					<?php } else { ?>
	...
	<?php // texte heavy
	if($memTooHeavyDisplayMessage === true) { ?>
		<div class="align_texte"><span class="message_heavy too_heavy"></span><span class='texte_heavy'><?=trans('too_heavy')?></span></div>
	<?php }
	if($memVeryHeavyDisplayMessage === true) { ?>
		<div class="align_texte"><span class="message_heavy very_heavy"></span><span class='texte_heavy'><?=trans('very_heavy')?></span></div>
	<?php } // /texte heavy ?>
<script>
